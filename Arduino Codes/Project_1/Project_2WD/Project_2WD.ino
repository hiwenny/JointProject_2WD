...
AccelStepper leftMotor(2, 2, 3);
AccelStepper rightMotor(2, 4, 5);
...
void loop() {
   ...
   int sampleCounter=0;
   int WDCounter=0;              //Watch Dog counter
  ...
  if (DelayValue < 255 ){// && sampleCounter > DelayValue) {
     ...
     mydata1.sampleNumber = samNum;
     mydata1.time = millis();
     samNum++;
     
     SendBinaryData(&mydata1, &msginfo);
     
     sampleCounter=0;
  }
 
  x = AskPendingSerialMessages();
  if(WDCounter>1000){ //might need to change number to be appropreciate
      WDFlag=1;
  }
  ...
  if (x == 1){
    ...
    WDCounter=0; //resets watch dog counter
    WDFlag=0; //resets watch dog flag
  } 
  ...
  if (analogRead(in1) < 850) {
    ...
    SendBinaryData(&batteryError, &msginfo); //might need to use a code
...
//       int rightDistance = int(p[4]);
         leftMotor.move((int)p[1]);
         rightMotor.move((int)p[2]);
          while(leftMotor.distanceToGo()!=0){ //WDFlag is Watch Dog Flag  && WDFlag==0 not working
